...
void getPlayerMove(int player, int size) {
    ...
    int r, c;
    for (;;) {
        printf("Player %d (%c) enter row and col (0-%d): ",
               player, player == 1 ? 'X' : 'O', gSize - 1);
        if (scanf("%d %d", &r, &c) != 2) { while (getchar() != '\n'); printf("Invalid input.\n"); continue; }
        if (!isValidMove(r, c, gSize)) { printf("Invalid move.\n"); continue; }
...
int checkWin(int size) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ok = 1; for (int j = 0; j < n; j++) if (board[j][i] != s) { ok = 0; break; } if (ok) return 1;
    }
    int ok = 1; for (int i = 0; i < n; i++) if (board[i][i] != s) { ok = 0; break; } if (ok) return 1;
    ok = 1; for (int i = 0; i < n; i++) if (board[i][n - 1 - i] != s) { ok = 0; break; } if (ok) return 1;
    return 0;
}
